package auth

import (
	"fmt"
	"log"
	"net/http"
	"os"

	"github.com/casbin/casbin/v2"
	"github.com/casbin/casbin/v2/model"

	"d2din.com/internal/pkg/code"
	"d2din.com/internal/pkg/jwe"

	//jsonadapter "github.com/casbin/json-adapter/v2"
	gormAdapter "github.com/casbin/gorm-adapter/v3"
	"github.com/gin-gonic/gin"
	"github.com/golang-jwt/jwt"
	_ "gorm.io/driver/postgres"
)

var Enforcer *casbin.Enforcer

func init() {
	Enforcer = Casbin()
}

// Add a Casbin rule from front.
type CasbinBind struct {
	Ptype    string `json:"ptype"`
	Rolename string `json:"rolename"`
	Path     string `json:"path"`
	Method   string `json:"method"`
}

type Claims struct {
	Account string `json:"username"`
	Role    string `json:"role"`
	jwt.StandardClaims
}

var jwtSecret = []byte("secret")

type CasbinModel struct {
	ID int `json:"id"`
	CasbinBind
}

func AddCasbin(cm CasbinModel) (bool, error) {
	return Enforcer.AddPolicy(cm.Rolename, cm.Path, cm.Method)
}

func DeleteCasbin(cm CasbinModel) (bool, error) {
	return Enforcer.RemovePolicy(cm.Rolename, cm.Path, cm.Method)
}

func GetAllPolice() [][]string {

	return Enforcer.GetPolicy()
}

func Casbin() *casbin.Enforcer {
	host := os.Getenv("SOURCES_HOST")
	port := os.Getenv("SOURCES_PORT")
	user := os.Getenv("SOURCES_USER")
	pass := os.Getenv("SOURCES_PASSWORD")
	dbname := os.Getenv("SOURCES_DATABASE")
	sslmode := os.Getenv("SOURCES_SSLMODE")

	dsn := "host=" + host + " user=" + user + " dbname=" + dbname + " port=" + port + " sslmode=" + sslmode + " password=" + pass
	log.Printf("Casbin conn: %s", dsn)

	adpter, err := gormAdapter.NewAdapter("postgres", dsn, true)
	if err != nil {
		panic(err)
	}
	// cb := []byte(_CASBIN_RULES)

	// a := jsonadapter.NewAdapter(&cb)

	m, errModel := model.NewModelFromString(`[request_definition]
	r = sub, obj, act
	
	[policy_definition]
	p = sub, obj, act
	
	[policy_effect]
	e = some(where (p.eft == allow))
	
	[matchers]
	m = r.sub == p.sub && keyMatch(r.obj, p.obj) && regexMatch(r.act, p.act)
	
	#[matchers]
	#m = r.sub == p.sub && r.obj == p.obj && r.act == p.act`)
	if errModel != nil {
		log.Fatalf("error: model; %s", errModel)
	}

	e, err := casbin.NewEnforcer(m, adpter)
	if err != nil {
		panic(err)
	}

	// 从DB加载策略
	e.LoadPolicy()

	return e
}

func AuthCheckRole() gin.HandlerFunc {
	return func(c *gin.Context) {
		privateKey := "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----"
		j := &jwe.JWT{
			// PrivateKey: os.Getenv("PRIVATE"),
			PrivateKey: privateKey,
			Token:      c.GetHeader("Authorization"),
		}
		if len(j.Token) == 0 {
			c.AbortWithStatusJSON(code.JWTRejected, code.GetCodeMessage(code.JWTRejected, "jwe is null"))
			return
		}

		j, err := j.Verify()
		if err != nil {

			c.AbortWithStatusJSON(code.JWTRejected, code.GetCodeMessage(code.JWTRejected, err.Error()))
			return
		}

		fmt.Println(j.Other["account_id"])

		role := j.Other["account_id"]

		e := Casbin()
		log.Printf("Casbin policy: %s,%s,%s", role, c.Request.URL.Path, c.Request.Method)

		res, err := e.Enforce(role, c.Request.URL.Path, c.Request.Method)
		log.Println(c.Request.URL.Path, c.Request.Method)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"status": -1,
				"msg":    "錯誤訊息" + err.Error(),
			})
			c.Abort()
			return
		}
		if res {
			c.Next()
		} else {
			c.JSON(http.StatusNonAuthoritativeInfo, gin.H{
				"status": 203,
				"msg":    "很抱歉你沒有此權限",
			})
			c.Abort()
			return
		}
	}
}

func CheckRole(role_id interface {},path string,action string)(status int) {

		fmt.Println(role_id)

		e := Casbin()
		log.Printf("Casbin policy: %s,%s,%s", role_id, path, action)

		res, err := e.Enforce(role_id, path, action)
		log.Println(path, action)
		if err != nil {
			// c.JSON(http.StatusInternalServerError, gin.H{
			// 	"status": -1,
			// 	"msg":    "錯誤訊息" + err.Error(),
			// })
			// c.Abort()
			return -1
		}
		if res {
			return 200
		} else {
			// c.JSON(http.StatusNonAuthoritativeInfo, gin.H{
			// 	"status": 203,
			// 	"msg":    "很抱歉你沒有此權限",
			// })
			// c.Abort()
			return 203
		}
}

