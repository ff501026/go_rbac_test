...
authority:
	GOOS=linux GOARCH=amd64 $(GO) build -o $(OUTPUTS)/authority cmd/authority/authority.go
	zip -D -j -r $(OUTPUTS)/authority.zip $(OUTPUTS)/authority

itax:
	GOOS=linux GOARCH=amd64 $(GO) build -o $(OUTPUTS)/itax cmd/itax/itax.go
	zip -D -j -r $(OUTPUTS)/itax.zip $(OUTPUTS)/itax

canjune:
	GOOS=linux GOARCH=amd64 $(GO) build -o $(OUTPUTS)/canjune cmd/canjune/canjune.go
	zip -D -j -r $(OUTPUTS)/canjune.zip $(OUTPUTS)/canjune
